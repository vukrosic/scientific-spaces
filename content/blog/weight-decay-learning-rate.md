...
$$
\boldsymbol{\theta}_t = \beta_3^t \boldsymbol{\theta}_{0} + (1 - \beta_3)\sum_{i=1}^t \beta_3^{t-i} (-\boldsymbol{u}_i / \lambda)
$$
...
$$
\text{RMS}(\boldsymbol{\theta}) \approx \sqrt{\frac{\eta}{2\lambda}}
$$
...
$$
\boldsymbol{\theta}_t \approx e^{-\kappa_t}\left(\boldsymbol{\theta}_0 - \sum_{i=1}^t e^{\kappa_i}\eta_i\boldsymbol{u}_i\right)
$$
...
$$
\lambda_s \eta_s + \frac{\dot{\eta}_s}{\eta_s} \approx 0
$$

If Weight Decay $\lambda$ is constant, the optimal learning rate schedule is:

$$
\eta_s \approx \frac{\eta_{\max}}{\lambda\eta_{\max} s + 1}
$$
